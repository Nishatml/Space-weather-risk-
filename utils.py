...
def fetch_dscovr_data():
    ...
    # 2-hour real-time endpoints for fast responses
    ...
    try:
        ...
        if p_res.status_code == 200 and m_res.status_code == 200:
            ...
            st.error(f"NOAA API Status Error: Plasma ({p_res.status_code}), Mag ({m_res.status_code})")
            return None
            
    except Exception as e:
        st.error(f"Live API Fetch Error: {e}")
        return None
